...
Scenario: valid interface
    Given we received a new zip file in the s3 bucket "r_inpu"
    When the file name matches the accepted interfaces in the interfaces.txt on the S3 bucket "r_config_inteface"
...
Scenario: not registered interface
    ...
    When the file name does not matches the accepted interfaces in the interfaces.txt on the S3 bucket "r_config_inteface"
